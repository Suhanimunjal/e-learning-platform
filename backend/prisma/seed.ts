...
async function main() {
  ...
  await prisma.quizAttempt.createMany({
    data: [
      { userId: student1.id, quizId: quiz1.id, answers: { q1: { answer: 'const' }, q2: { answer: 'object' }, q3: { answer: '//' }, q4: { answer: 'push()' } } as any, score: 30, percentage: 60, passed: false, startedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), completedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000 + 10 * 60 * 1000) },
      { userId: student2.id, quizId: quiz1.id, answers: { q1: { answer: 'const' }, q2: { answer: 'object' }, q3: { answer: '//' }, q4: { answer: 'push()' } } as any, score: 50, percentage: 100, passed: true, startedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000), completedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000 + 12 * 60 * 1000) },
    ],
  });
  ...
  const plugins = [
    { pluginId: 'plugin-quiz-system', name: 'Quiz System', category: 'CONTENT' as const, enabled: true },
    { pluginId: 'plugin-question-bank', name: 'Question Bank', category: 'CONTENT' as const, enabled: true },
    { pluginId: 'plugin-content-library', name: 'Content Library', category: 'CONTENT' as const, enabled: true },
    { pluginId: 'plugin-forum', name: 'Forum System', category: 'COMMUNICATION' as const, enabled: true },
    { pluginId: 'plugin-analytics', name: 'Learner Analytics', category: 'ANALYTICS' as const, enabled: true },
    { pluginId: 'plugin-payments', name: 'Razorpay Payments', category: 'PAYMENTS' as const, enabled: true },
    { pluginId: 'plugin-subscriptions', name: 'Subscription Management', category: 'PAYMENTS' as const, enabled: true },
    { pluginId: 'plugin-user-management', name: 'User Management', category: 'USERMGMT' as const, enabled: true },
    { pluginId: 'plugin-ai-content', name: 'AI Content Generation', category: 'AI' as const, enabled: true },
    { pluginId: 'plugin-certificates', name: 'Certificate Generator', category: 'UTILITY' as const, enabled: true },
  ];
